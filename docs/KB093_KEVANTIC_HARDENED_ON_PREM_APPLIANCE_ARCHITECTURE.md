# KB-093 — Kevantic Hardened On-Prem Appliance Architecture

Status: **Architecture / design** (scaffold under `kevantic-appliance/`).  
Module type: Documentation + repository layout — **no control-plane runtime changes** in this KB.  
Brand: **Kevantic** (platform, CLI, licensing, cloud SOC). Base OS remains **Ubuntu Server LTS** (not rebranded).
...
Define the architecture for the **Kevantic Hardened On-Prem Appliance**: a turnkey, aggressively minimized Ubuntu Server image that:

1. Collects and processes customer telemetry **locally**
2. Retains sensitive log payloads on-prem per customer preference
3. Forwards **only structured metadata and critical high-fidelity alerts** to Kevantic Cloud SOC
...
This KB is the design contract for ISO builds, the outbound control channel, OTA/WPK staging, and `kevantic-cli`.
...
| Layer | Owns |
|-------|------|
| Kevantic Admin / Customer portals (VM 100 control plane) | Human UX, entitlements commerce, activation tokens, **centralized casework / ticketing UI** |
| Kevantic Cloud SOC (`soc.kevantic.com`) | Central triage, **TheHive / case platform**, mTLS CA, signed license issuance, OTA repository |
| **This appliance** | Local sensors, parsers, staging, Wazuh Manager (customer-site), channel agent, wipe |
| Customer endpoints | **Only** native `wazuh-agent` — no second Kevantic endpoint agent |
...
|-----------|----------------|
| **TheHive** (or any on-prem ticketing/case UI) | Casework is **centralized** in Kevantic Cloud SOC only |
...
A **single** artifact — `Kevantic-Appliance-vX.Y.iso` — covers every appliance contract:
...
| Cloud endpoints (AWS / Azure / GCP) + appliance (`cloud_appliance`) | Same ISO as VM (customer or Kevantic-managed VPC) or physical edge; agents point at appliance reachability path |
...
|--------|----------------------------|--------------|
| **A — Physical / factory pre-stage** | Kevantic office builds or images the box **before** shipping to customer | Engineer runs install + **first-time bootstrap updates** on Kevantic network → ship → at customer site: LAN + SOC channel (bootstrap already done unless re-run is required) |
...
|---------|------|-----------------|
| **BOOTSTRAP** | First install / first critical patch cycle (office or customer site) | Temporary **outbound Internet** for: Ubuntu LTS **critical security** updates; Kevantic-approved **backend engine** critical updates (Wazuh Manager, Suricata/Zeek packages or images, Fluent Bit, container runtime, etc.); DNS/NTP as needed; registration to SOC |
| **LOCKED** (steady state) | After bootstrap reports success and `kevantic-cli network lock` (or equivalent) | **No general Internet** inbound or outbound. Allowed: (1) **internal/private** receive of endpoint (and permitted collector) alerts on LAN/VPC; (2) **outbound secure channel** to Kevantic MSSP SOC for metadata + selected critical high-fidelity alerts (+ control/OTA over that same channel). Everything else denied |
...
CLI markers (see CLI spec): `kevantic-cli bootstrap update` → `kevantic-cli network lock` → `status` shows `network_mode: locked`.
...
| **B — Channel v1 (target)** | `wss://soc.kevantic.com/appliance/v1/channel` | Mutual TLS client cert + appliance UUID | Bidirectional frames; heartbeat becomes channel ping + status publish |
...
1. Field engineer burns ISO, runs `kevantic-cli setup --token …`
...
4. Kevantic CA issues **client certificate** bound to appliance UUID + fingerprint claims (SAN/URI)
5. Private key stored in TPM (preferred) or file under `/var/lib/kevantic/secrets/` (0600, AppArmor confined)
...
Schemas live in `kevantic-appliance/channel/schemas/`.
...
- **OUTPUT:** allow DNS, NTP, HTTPS to Ubuntu security mirrors / Kevantic-approved package & image endpoints / `soc.kevantic.com`; optional customer HTTP(S) proxy
...
- **OUTPUT:** **deny general Internet**; allow **only** the Kevantic SOC secure channel (`soc.kevantic.com:443` mTLS / WSS) for metadata, selected critical alerts, heartbeat/control/OTA; DNS/NTP only if required to reach that channel (prefer pinning / local chrony already synced)
- No cloud-initiated inbound TCP into the customer perimeter
- Re-opening BOOTSTRAP requires `kevantic-cli network unlock --yes` (audited break-glass), never silent

State file: `/var/lib/kevantic/network_mode` ∈ `bootstrap` \| `locked`.
...
2. Engineer uses **the same** `Kevantic-Appliance-vX.Y.iso`:
   - **Method A:** image physical hardware at Kevantic office, **or**
   - **Method B:** create VM on customer hypervisor / cloud VPC from the same ISO
3. Autoinstall + first boot → `kevantic-cli setup` (token, name, network)
4. **`kevantic-cli bootstrap update`** — temporary Internet: Ubuntu critical security + approved backend engine critical updates; verify success
5. **`kevantic-cli network lock`** — steady-state: LAN/VPC agents in; SOC channel out only
...
Pre-staged physical units may complete steps 4–5 at the Kevantic office so customer-site work is mainly LAN integration + SOC channel verify. If the unit sat long enough that new critical patches shipped, engineer re-runs bootstrap **before** lock (or unlock → update → lock).

### 5.2 Offboarding & cryptographic decommission

Triggered from Admin portal **or** `kevantic-cli offboard` / `wipe`:

1. Control plane revokes client cert at Kevantic CA; marks appliance `retired` (KB-016 heartbeat already returns 403 for retired)
2. Appliance stops services; deletes key material; shreds secret volumes
3. Purges `/var/lib/kevantic` data pools and local logs per retention policy
...
Subscription changes in Kevantic Portal → signed **entitlement payload** → appliance.
...
```bash
kevantic-cli enable-service --key <SIGNED_BLOB>
```
...
Implemented as `kevantic-license-enforcer` (Ansible role `license_enforcer`):

- Maintain trust store of Kevantic verify keys
...
Cloud OTA repo (signed manifests)
    ...
    --> stage under /var/lib/kevantic/ota/
...
1. Appliance pulls verified Wazuh Agent WPK from Kevantic cloud repo over channel/mTLS
2. Stages under `/var/lib/kevantic/wpk/`
...
| svc-06 | NDR | Suricata + Zeek (Kevantic-wrapped), SPAN/TAP capture |
...
Module definitions: `kevantic-appliance/services/NN-*/`.
...
4. **Mask** systemd units not required for Kevantic runtime or OS stability  
...
8. Install `kevantic-cli`, `kevantic-channeld`, core svc-01  
9. Shrink → produce `Kevantic-Appliance-vX.Y.iso`
...
Canonical tree: `/opt/mssp-control/kevantic-appliance/` — see `kevantic-appliance/docs/REPO_LAYOUT.md`.

CLI contract: `kevantic-appliance/docs/KEVANTIC_CLI_SPEC.md`.
...
**Appliance Management must move off the `mssp-control` server onto a dedicated Appliance Management plane** (a separate server) before/at production scale. Do not permanently couple channel gateway, appliance CA, OTA/WPK repository, or high-volume appliance ingress to the control-plane host.
...
| **Appliance Management Plane** | **VM 114** `kevantic-appliance-mgmt` (`192.168.0.224`) — live; see `docs/KB093L_APPLIANCE_MANAGEMENT_PLANE_VM114.md` | `soc.kevantic.com` edge for appliances: mTLS channel gateway, appliance CA issue/revoke, OTA + WPK repo, registration/bootstrap update allow-lists, appliance health fan-in |
...
| Kevantic CA service | Issue/revoke appliance client certs | Appliance Management plane |
...
- **No TheHive / ticketing stack on the appliance** — cases stay in Kevantic Cloud SOC
...
- `kevantic-appliance/` — build repo scaffold + docs
- `docs/KB093_KEVANTIC_HARDENED_ON_PREM_APPLIANCE_ARCHITECTURE.md` — this file
- `scripts/kb093_validate_kevantic_appliance_architecture.sh`
...
```bash
cd /opt/mssp-control
chmod +x scripts/kb093_validate_kevantic_appliance_architecture.sh
./scripts/kb093_validate_kevantic_appliance_architecture.sh
```
...
```text
KB-093 KEVANTIC HARDENED ON-PREM APPLIANCE ARCHITECTURE VALIDATION PASSED
```
...
| **B1** | `kevantic-cli` stub + network mode state + nftables profiles + Ansible `minimize` | Done |
