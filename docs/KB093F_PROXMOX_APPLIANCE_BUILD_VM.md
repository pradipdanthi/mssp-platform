# KB-093F — NikTiar Appliance build on Proxmox (replaces nested Packer default)

Status: Active build path  
Date: 2026-08-04  
Audience: Operator building `NikTiar-Appliance-vX.Y` field artifacts
...
| Role | Where |
|------|--------|
| Factory disk / install target | Proxmox **VM 113** `niktiar-appliance-build` (`192.168.0.223`) |
...
./niktiar-appliance/scripts/b2_proxmox_create_build_vm.sh

# 2) Sync roles to VM 112 and provision the build VM (Ansible on 112)
./niktiar-appliance/scripts/b2_proxmox_provision.sh

# 3) Export qcow2/raw/delivery ISO under niktiar-appliance/.cache/dist/
./niktiar-appliance/scripts/b2_proxmox_export_artifacts.sh
```
...
- Export: artifacts `NikTiar-Appliance-v*.{qcow2,raw,iso}` + `SHA256SUMS`

## What stays on VM 100

- Source: `niktiar-appliance/` (Ansible, CLI, engine, autoinstall reference `packer/http/`)
...
- `./niktiar-appliance/scripts/b2_packer_build.sh` — nested QEMU inside VM 100 (legacy; do not use unless explicitly requested)
- Docker image `niktiar-appliance-b2-builder:local` — cleared with ISO-build residuals

## Cleanup (ISO-build residuals only)

Safe to remove anytime:

- `niktiar-appliance/.cache/output/`
- `niktiar-appliance/.cache/logs/`
- `niktiar-appliance/.cache/dist/` (after you copied artifacts elsewhere)
- Docker image `niktiar-appliance-b2-builder:local`

Keep:

- `niktiar-appliance/` source tree
