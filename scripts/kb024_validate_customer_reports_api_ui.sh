#!/usr/bin/env bash
# KB-024: Validate Customer Reports API + Customer Reports Page.
...
BODY_FILE="/tmp/kb024-body.txt"
LOGIN_FILE="/tmp/kb024-login.json"

cd "$PROJECT_DIR"

echo "======================================================================"
echo "KB-024: Validate Customer Reports API and Customer Reports Page"
...
REQUIRED=(
  ...
  "docs/KB024_CUSTOMER_REPORTS_API_UI.md"
)
...
for p in frontend-admin/ postgres/init/ docker-compose.yml; do
  git diff --quiet -- "$p" || fail "$p has working-tree changes but KB-024 must not modify it"
  git diff --cached --quiet -- "$p" || fail "$p has staged changes but KB-024 must not modify it"
...
section "3. Backend source: reports endpoint"

grep -q '@router.get("/reports/{short_code}")' backend-api/app/api/routes/customer.py \
  || fail "customer.py missing GET /reports/{short_code}"
grep -q 'monthly_reports' backend-api/app/api/routes/customer.py \
  || fail "customer reports query missing monthly_reports"
grep -q 'require_tenant_match' backend-api/app/api/routes/customer.py \
  || fail "customer.py missing require_tenant_match"
echo "OK: customer reports route present."

section "4. Frontend: getCustomerReports + no /admin"
...
grep -q 'getCustomerReports' frontend-customer/src/pages/ReportsPage.tsx \
  || fail "ReportsPage.tsx must call getCustomerReports"
grep -q 'getCustomerDashboard' frontend-customer/src/pages/ReportsPage.tsx \
  && fail "ReportsPage.tsx must not call getCustomerDashboard anymore"

if grep -REn '/admin' frontend-customer/src 2>/dev/null; then
  fail "frontend-customer/src must not contain /admin"
fi
echo "OK: frontend uses /customer/reports and has no /admin string."
...
section "6. OpenAPI lists /customer/reports/{short_code}"

OPENAPI="$(curl -fsS "$API_BASE/openapi.json" || fail "Could not fetch OpenAPI")"
echo "$OPENAPI" | jq -e '.paths | has("/customer/reports/{short_code}")' >/dev/null \
  || fail "OpenAPI missing /customer/reports/{short_code}"
echo "OK: OpenAPI registers /customer/reports/{short_code}."

section "7. Unauthenticated access returns 401"

HTTP_CODE="$(curl -sS -o "$BODY_FILE" -w "%{http_code}" "$API_BASE/customer/reports/DEMO" || true)"
[ "$HTTP_CODE" = "401" ] || fail "Unauthenticated GET /customer/reports/DEMO expected 401, got $HTTP_CODE"
...
section "9. customer.viewer can call /customer/reports/DEMO"

HTTP_CODE="$(curl -sS -o "$BODY_FILE" -w "%{http_code}" \
  -H "Authorization: Bearer $CUSTOMER_VIEWER_TOKEN" \
  "$API_BASE/customer/reports/DEMO" || true)"
[ "$HTTP_CODE" = "200" ] || fail "GET /customer/reports/DEMO expected 200, got $HTTP_CODE ($(cat "$BODY_FILE"))"

jq -e '.tenant.short_code == "DEMO"' "$BODY_FILE" >/dev/null \
  || fail "DEMO reports response missing tenant.short_code DEMO"
jq -e '.reports | type == "array"' "$BODY_FILE" >/dev/null \
  || fail "DEMO reports response missing reports array"
assert_no_forbidden_payload "DEMO reports" "$BODY_FILE"
echo "OK: DEMO reports response shape is customer-safe (reports=$(jq '.reports | length' "$BODY_FILE"))."

section "10. customer.viewer cannot call /customer/reports/DEMO2 (expect 404)"

HTTP_CODE="$(curl -sS -o "$BODY_FILE" -w "%{http_code}" \
  -H "Authorization: Bearer $CUSTOMER_VIEWER_TOKEN" \
  "$API_BASE/customer/reports/DEMO2" || true)"
[ "$HTTP_CODE" = "404" ] || fail "GET /customer/reports/DEMO2 as DEMO viewer expected 404, got $HTTP_CODE"
echo "OK: cross-tenant reports access returns 404."
...
[ -f "docs/KB024_CUSTOMER_REPORTS_API_UI.md" ] || fail "docs/KB024_CUSTOMER_REPORTS_API_UI.md missing"
grep -q 'GET /customer/reports' docs/KB024_CUSTOMER_REPORTS_API_UI.md \
...
echo "open Reports, and confirm a read-only list or empty state (no admin APIs)."

section "14. Final verdict"

echo "======================================================================"
echo "KB-024 CUSTOMER REPORTS API UI VALIDATION PASSED"
echo "======================================================================"
