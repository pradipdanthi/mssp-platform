...
grep -q 'asset_category' backend-api/app/api/routes/customer.py || fail "customer alerts asset_category filter missing"
grep -q 'list-device-type-filter' frontend-admin/src/components/soc/SocFilterBar.tsx || fail "admin Device type dropdown missing"
grep -q 'list-device-type-filter' frontend-customer/src/components/soc/SocFilterBar.tsx || fail "customer Device type dropdown missing"
grep -q 'showDeviceTypeFilter' frontend-admin/src/pages/AlertsPage.tsx || fail "admin AlertsPage device-type wiring missing"
grep -q 'showDeviceTypeFilter' frontend-customer/src/pages/AlertsPage.tsx || fail "customer AlertsPage device-type wiring missing"
...
! grep -q 'AlertTaxonomyNav' frontend-admin/src/pages/AlertsPage.tsx || fail "AlertsPage still mounts AlertTaxonomyNav sidebar"
! grep -rq 'Device taxonomy' frontend-admin/src frontend-customer/src --include='*.tsx' || fail "Device taxonomy sidebar label still present"

python3 -m py_compile backend-api/app/services/soc_alert_taxonomy.py || fail "py_compile taxonomy"
python3 -m py_compile backend-api/app/api/routes/customer.py || fail "py_compile customer routes"
