...
# Set JUNEXIS_B2_FULL=1 to also run Packer QEMU build after ISO is present.
set -euo pipefail
...
if grep -qiE 'separate server|Appliance Management Plane' \
  "$ROOT/docs/KB093_JUNEXIS_HARDENED_ON_PREM_APPLIANCE_ARCHITECTURE.md"; then
...
if "$ROOT/scripts/kb093b_validate_junexis_cli_b1.sh" >/tmp/kb093b-out.txt; then
  pass "B1 regression"
else
  fail "B1 regression"
...
if [[ "${JUNEXIS_B2_FULL:-0}" == "1" ]]; then
  echo "--- FULL Packer QEMU build (JUNEXIS_B2_FULL=1) ---"
  ...
  pass "skipped full ISO build (set JUNEXIS_B2_FULL=1 to enable)"
