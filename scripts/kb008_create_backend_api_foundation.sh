...
echo "======================================================================"
echo "KB-008: Create Backend API Foundation"
echo "Target: $PROJECT_DIR"
echo "Purpose: Create FastAPI backend service connected to PostgreSQL and Redis"
echo "======================================================================"

echo
echo "1. Pre-check project foundation"
[ -f "$PROJECT_DIR/docker-compose.yml" ] || { echo "ERROR: docker-compose.yml missing"; exit 1; }
[ -f "$PROJECT_DIR/.env" ] || { echo "ERROR: .env missing"; exit 1; }
[ -d "$PROJECT_DIR/postgres/init" ] || { echo "ERROR: postgres/init missing"; exit 1; }

docker compose ps postgres redis

echo
echo "2. Add API settings to .env if missing"
grep -q '^APP_ENV=' .env || echo 'APP_ENV=development' >> .env
grep -q '^API_PORT=' .env || echo 'API_PORT=8000' >> .env
chmod 600 .env

echo
echo "3. Create backend-api folder structure"
...
echo
echo "4. Back up and rewrite docker-compose.yml with backend-api service"
backup_file="docker-compose.yml.backup-kb008-$(date +%Y%m%d-%H%M%S)"
...
services:
  ...
  backend-api:
    build:
      context: ./backend-api
    container_name: mssp-backend-api
    restart: unless-stopped
    environment:
      APP_NAME: MSSP Control Plane API
      APP_ENV: ${APP_ENV}
...
echo
echo "5. Validate Docker Compose configuration"
docker compose config >/tmp/kb008-compose-check.txt
echo "Docker Compose validation passed"

echo
echo "6. Build and start backend-api"
docker compose up -d --build backend-api

echo
echo "7. Wait for backend-api health endpoint"
for i in $(seq 1 60); do
  if curl -fsS http://localhost:8000/health >/tmp/kb008-health.json 2>/dev/null; then
    echo "backend-api health endpoint is responding"
    cat /tmp/kb008-health.json
    echo
    break
  fi

  if [ "$i" -eq 60 ]; then
    echo "ERROR: backend-api did not become healthy within 60 seconds"
    docker compose logs --tail=80 backend-api
    exit 1
  fi
...
echo
echo "8. Current Docker services"
docker compose ps
...
echo "From LAN browser:"
echo "  http://192.168.0.201:8000/health"
echo "  http://192.168.0.201:8000/docs"
echo
echo "KB-008 backend API foundation creation completed."
