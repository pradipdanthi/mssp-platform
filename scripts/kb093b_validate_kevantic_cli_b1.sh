#!/usr/bin/env bash
# KB-093 B1 — kevantic-cli + nft profiles + minimize role smoke (no root apt/nft required)
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CLI_DIR="$ROOT/kevantic-appliance/cli/kevantic-cli"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT

export KEVANTIC_STATE_DIR="$TMP/state"
export KEVANTIC_CONFIG_DIR="$TMP/config"
...
echo "=== KB-093 B1 kevantic-cli / bootstrap-lock validation ==="

need() {
  [[ -f "$1" ]] && pass "file ${1#"$ROOT"/}" || fail "missing ${1#"$ROOT"/}"
}

need "$ROOT/kevantic-appliance/hardening/nftables/bootstrap.nft"
need "$ROOT/kevantic-appliance/hardening/nftables/locked.nft"
need "$ROOT/kevantic-appliance/ansible/roles/minimize/tasks/main.yml"
need "$ROOT/kevantic-appliance/cli/kevantic-cli/kevantic_cli/cli.py"
need "$ROOT/docs/KB093_KEVANTIC_HARDENED_ON_PREM_APPLIANCE_ARCHITECTURE.md"

if grep -Fq "Separate server" "$ROOT/docs/KB093_KEVANTIC_HARDENED_ON_PREM_APPLIANCE_ARCHITECTURE.md" \
  || grep -Fq "separate server" "$ROOT/docs/KB093_KEVANTIC_HARDENED_ON_PREM_APPLIANCE_ARCHITECTURE.md"; then
  pass "KB-093 documents separate Appliance Management server"
else
  fail "KB-093 missing separate Appliance Management server note"
fi

if grep -Fq "TheHive" "$ROOT/kevantic-appliance/ansible/roles/minimize/tasks/main.yml"; then
  fail "minimize role must not reference installing TheHive"
else
  pass "minimize role does not install TheHive"
fi

CLI=(python3 -m kevantic_cli)
...
MODE="$(cat "$KEVANTIC_STATE_DIR/network_mode")"
...
RESULT="$(python3 -c "import json;print(json.load(open('$KEVANTIC_STATE_DIR/bootstrap.json'))['last_result'])")"
...
need "$ROOT/kevantic-appliance/configs/systemd/kevantic-heartbeat.service"
need "$ROOT/kevantic-appliance/configs/systemd/kevantic-heartbeat.timer"

# Token must not be stored raw
if grep -Rq "test-token-not-real-abcdefgh" "$KEVANTIC_STATE_DIR"; then
...
"$ROOT/scripts/kb093_validate_kevantic_appliance_architecture.sh" >/tmp/kb093-out.txt
tail -1 /tmp/kb093-out.txt | grep -q PASSED && pass "kb093 architecture validator" || fail "kb093 architecture validator"

if [[ "$FAIL" -ne 0 ]]; then
  echo "KB-093 B1 KEVANTIC CLI BOOTSTRAP-LOCK VALIDATION FAILED"
  exit 1
fi
echo "KB-093 B1 KEVANTIC CLI BOOTSTRAP-LOCK VALIDATION PASSED"
