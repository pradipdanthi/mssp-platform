...
export default function IncidentDetailPanel({
  ...
  const affected =
    "affected_entity" in incident
      ? (incident as { affected_entity?: string | null }).affected_entity
      : null;
  const host =
    affected && affected !== "Pending enrichment"
      ? affected
      : mode === "admin"
        ? "host-pending"
        : "Protected asset";
  const adminIp =
    "source_ip" in incident ? (incident as { source_ip?: string | null }).source_ip : null;
  const ipLabel =
    mode === "admin" && adminIp
      ? adminIp
      : mode === "admin"
          ...
          <button
            ...
            onClick={() => {
              if (onRunPlaybook) onRunPlaybook();
                ...
                window.alert(
                  "Playbook run queued for security automation. Status updates appear when your SOC publishes them to this portal."
                );
            }}
          >
            Run Playbook
