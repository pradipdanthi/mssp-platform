...
readiness/heartbeat loop and claims jobs from junexis-engine-api.
"""
...
ENGINE_API = os.environ.get("JUNEXIS_ENGINE_API", "http://127.0.0.1:8787")

# Backend tools that must exist on disk for each catalogue service
REQUIRED = {
    "svc-01": ["wazuh-manager", "fluent-bit"],
    "svc-02": ["junexis-engine-api"],
    "svc-03": ["junexis-engine-api"],
    "svc-04": ["nuclei", "vuls"],
    "svc-05": ["wazuh-manager"],
    "svc-06": ["suricata", "zeek"],
    "svc-07": ["junexis-engine-api"],
    "svc-08": ["junexis-engine-api"],
    "svc-09": ["nuclei", "junexis-engine-api"],
    "svc-10": ["junexis-engine-api"],
}
...
def _which(name: str) -> str | None:
    if name == "junexis-engine-api":
        for p in ("/usr/bin/junexis-engine-api", "/usr/local/bin/junexis-engine-api"):
            if Path(p).exists():
                return p
...
def ensure_backend_running(svc: str) -> None:
    if svc == "svc-01":
        ...
        subprocess.run(["systemctl", "start", "junexis-appliance-engine"], check=False)
