#!/usr/bin/env bash
# Create disposable Proxmox build VM 113 (Junexis appliance factory).
# Does NOT run nested Packer on VM 100.
set -euo pipefail
...
VM_NAME="${JUNEXIS_BUILD_VM_NAME:-junexis-appliance-build}"
VM_IP="${JUNEXIS_BUILD_VM_IP:-192.168.0.223}"
VM_CIDR="${JUNEXIS_BUILD_VM_CIDR:-24}"
VM_GW="${JUNEXIS_BUILD_VM_GW:-192.168.0.1}"
VM_MEM="${JUNEXIS_BUILD_VM_MEM:-6144}"
VM_CORES="${JUNEXIS_BUILD_VM_CORES:-2}"
VM_DISK="${JUNEXIS_BUILD_VM_DISK:-40}"
BRIDGE="${JUNEXIS_BUILD_VM_BRIDGE:-vmbr0}"
STORAGE="${JUNEXIS_BUILD_VM_STORAGE:-local-zfs}"
CLOUDIMG="${JUNEXIS_BUILD_CLOUDIMG:-/var/lib/vz/template/iso/ubuntu-24.04-server-cloudimg-amd64.img}"
SSH_PUB="${JUNEXIS_BUILD_SSH_PUB:-$ROOT/.tools/build-ssh/junexis_packer.pub}"
...
scp -o BatchMode=yes -i "$PVE_KEY" "$SSH_PUB" "root@${PVE_HOST}:/tmp/junexis_build_vm.pub"
...
qm set "\$VMID" --sshkeys /tmp/junexis_build_vm.pub
qm set "\$VMID" --ciupgrade 0
rm -f /tmp/junexis_build_vm.pub
