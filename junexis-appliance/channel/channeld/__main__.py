"""junexis-channeld — outbound SOC channel client (Phase B).
...
LOG = logging.getLogger("junexis-channeld")
...
def _config_root() -> Path:
    return Path(os.environ.get("JUNEXIS_CONFIG_DIR", "/etc/junexis"))
...
def load_creds() -> tuple[str, str]:
    ...
    appliance_id = os.environ.get("JUNEXIS_APPLIANCE_ID", appliance_id)
    api_key = os.environ.get("JUNEXIS_APPLIANCE_API_KEY", "")
...
def _ssl_context() -> ssl.SSLContext:
    verify = (os.environ.get("JUNEXIS_TLS_VERIFY", "true") or "true").lower() not in (
        "0",
        "false",
        "no",
    )
...
def handle_frame(frame: dict[str, Any], *, base: str, headers: dict[str, str]) -> dict[str, Any]:
    ...
    if ftype == "license.push":
        ...
        try:
            from junexis_cli.license_ops import apply_license_file
    ...
    if ftype == "ota.offer":
        try:
            from junexis_ota import apply_offer
    ...
    frames.append(
        {
            ...
            "id": str(uuid4()),
            "ts": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
            "payload": {"source": "junexis-channeld", "ok": True},
        }
    )
    http_json(
        "POST",
...
def run_poll_loop(cfg: dict[str, Any]) -> int:
    ...
    headers = {
        ...
        "User-Agent": "junexis-channeld/0.1",
    }
    interval = int(cfg.get("poll_interval_sec") or os.environ.get("JUNEXIS_CHANNEL_POLL_SEC") or 20)
...
def run_ws_loop(cfg: dict[str, Any]) -> int:
            ...
            json.dumps(
                {
                    ...
                    "type": "heartbeat",
                    "id": str(uuid4()),
                    "payload": {"source": "junexis-channeld", "hello": True},
                }
            )
        )
    ...
    sslopt = {}
    if (os.environ.get("JUNEXIS_TLS_VERIFY", "true") or "true").lower() in ("0", "false", "no"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(prog="junexis-channeld")
    p.add_argument("--config", default="/etc/junexis/channel.yaml")
