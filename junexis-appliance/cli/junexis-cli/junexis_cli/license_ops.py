"""Apply / show Junexis signed licenses on the appliance."""
...
def _pubkey_candidates() -> list[Path]:
    env = os.environ.get("JUNEXIS_LICENSE_PUBKEY", "").strip()
    out: list[Path] = []
    if env:
        out.append(Path(env))
    out.extend(
...
def find_pubkey() -> Path:
    ...
    raise FileNotFoundError(
        "No licensing public key found under /etc/junexis/trust/keys/"
    )
...
def verify_and_parse(token: str, pubkey_path: Path, fingerprint: str = "") -> dict[str, Any]:
    ...
    if claims.get("iss") != "junexis-license":
...
def apply_license_file(path: Path, fingerprint: str = "") -> dict[str, Any]:
    ...
    if Path("/usr/bin/junexis-reconcile-services").is_file():
        proc = subprocess.run(
