# Junexis Appliance — Repository Layout
...
│   ├── JUNEXIS_CLI_SPEC.md          # CLI contract
...
│       ├── junexis_runtime/         # dirs, users, base units
...
│   ├── apparmor/                    # profiles for junexis-* daemons
│   ├── nftables/
│   │   └── junexis-appliance.nft
│   └── auditd/
│       └── junexis.rules
├── services/                        # one folder per Junexis service module
...
├── cli/
│   └── junexis-cli/                 # Go (preferred) or Python CLI sources
│       ├── cmd/
...
| Item | Rule |
|------|------|
| ISO | `Junexis-Appliance-v{MAJOR}.{MINOR}.iso` |
| Service id | `svc-01` … `svc-10` (stable; never rename) |
| Systemd units | `junexis-channeld.service`, `junexis-svc-NN-*.service` |
| Container images | `registry.junexis.com/appliance/<svc>:<version>` (pulled over mTLS channel or signed OTA) |
| Config root | `/etc/junexis/` |
| Data root | `/var/lib/junexis/` |
| Secrets | `/var/lib/junexis/secrets/` (0600, not in Git) |
| OTA cache | `/var/lib/junexis/ota/` |
| WPK cache | `/var/lib/junexis/wpk/` |
...
This `junexis-appliance/` tree is the **full hardened ISO product** that replaces that placeholder for production field deployment. Until the first ISO ship, field use may still start from the KB-058 template for lab registration tests.
