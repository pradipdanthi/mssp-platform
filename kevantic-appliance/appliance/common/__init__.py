...
def log_root() -> Path:
    return Path(os.environ.get("KEVANTIC_LOG_DIR", "/var/log/kevantic"))


def datalake_root() -> Path:
    return Path(os.environ.get("KEVANTIC_DATALAKE_DIR", str(log_root() / "datalake")))


def metadata_db_path() -> Path:
    return Path(
        os.environ.get(
            "KEVANTIC_METADATA_DB",
...
def telemetry_url() -> str:
    return os.environ.get(
        ...
        "https://api.kevantic.com/api/v1/telemetry/ingest",
    )
...
def hunt_callback_url() -> str:
    return os.environ.get(
        ...
        "https://api.kevantic.com/api/v1/telemetry/hunt-results",
    )
