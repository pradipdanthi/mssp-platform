...
Exposes kevantic-engine-api endpoints used by svc-02..10 workers and cloud-pushed jobs.
"""
...
def serve(host: str | None = None, port: int | None = None) -> None:
    ...
    """
    host = host or os.environ.get("KEVANTIC_ENGINE_API_HOST", "127.0.0.1")
    port = int(port or os.environ.get("KEVANTIC_ENGINE_API_PORT", "8787"))
    job_queue.write_status_marker()
    httpd = ThreadingHTTPServer((host, port), ApplianceAPIHandler)
    print(f"kevantic-engine-api on http://{host}:{port}")
    httpd.serve_forever()
