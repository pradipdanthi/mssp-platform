#!/usr/bin/env python3
"""Generate Kevantic Appliance boot splash (1920x1080).

Branding (locked):
  - KEVANTIC APPLIANCE — yellow→red mix (website amber #F59E0B + deep red)
  - Your Dedicated SOC Sentinel — white
  - Pure black background, Proxmox-style minimal
  - Includes improvised radar mark inspired by website-kevantic/assets/mark.svg

Usage:
  python3 generate_boot_logo.py [/path/to/output.png]
Default output (dev tree):
  ./kevantic-boot.png  (next to this script)
On appliance firstboot / chroot:
  /usr/share/plymouth/themes/kevantic/kevantic-boot.png
"""
...
# Website palette (website-kevantic/css/styles.css)
...
def generate(out_path: Path) -> Path:
    ...
    draw_brand_gradient_text(img, "KEVANTIC APPLIANCE", (960, 520), brand_font)
...
def main() -> None:
    if len(sys.argv) > 1:
        ...
        appliance_default = Path("/usr/share/plymouth/themes/kevantic/kevantic-boot.png")
        out = appliance_default if os.geteuid() == 0 and str(here).startswith("/usr") else here / "kevantic-boot.png"
    path = generate(out)
