...
Kevantic-native workers (svc-02/03/07/08/10) install from payload source and
`/usr/bin/kevantic-engine-worker` — no third-party .deb required.

Firstboot installs everything, then **disables** all catalogue units until
`kevantic-cli license apply`.
