#!/usr/bin/env bash
# RETIRED as primary customer media (KB-093N). Use mkosi/build.sh instead.
# Build a *bootable Ubuntu autoinstall ISO* for the Kevantic appliance (customer media).
# This is NOT a qcow2 delivery disc — it boots Subiquity and installs the OS.
set -euo pipefail

if [[ "${KEVANTIC_FORCE_LEGACY_REMASTER:-}" != "1" ]]; then
  cat <<'EOF' >&2
ERROR: Subiquity remaster ISO is retired (KB-093N immutable image strategy).

  Build:  /opt/mssp-control/kevantic-appliance/mkosi/build.sh
  Doc:    /opt/mssp-control/docs/KB093N_IMMUTABLE_APPLIANCE_IMAGE_STRATEGY.md

To run this legacy remaster anyway (not for customer ship):
  KEVANTIC_FORCE_LEGACY_REMASTER=1 $0
EOF
  exit 99
fi
...
CACHE="$ROOT/.cache"
ISO_IN="${KEVANTIC_UBUNTU_ISO:-$CACHE/ubuntu-24.04.4-live-server-amd64.iso}"
OUT_DIR="$CACHE/dist-install"
OUT_ISO="$OUT_DIR/Kevantic-Appliance-Install-v${VER}.iso"
WORK="$CACHE/iso-work"
BUILDER_IMAGE="${KEVANTIC_B2_BUILDER_IMAGE:-kevantic-appliance-b2-builder:local}"
...
tar -C "$ROOT" \
  ...
  | tar -C "$WORK/seed/kevantic-payload" -xf -

cp -f "$ROOT/iso/firstboot/kevantic-firstboot.sh" "$WORK/seed/kevantic-payload/kevantic-firstboot.sh"
cp -f "$ROOT/iso/firstboot/kevantic-firstboot.service" "$WORK/seed/kevantic-payload/kevantic-firstboot.service"
chmod +x "$WORK/seed/kevantic-payload/kevantic-firstboot.sh" "$ROOT/iso/docker_remaster.sh"

# Boot splash artwork + generator (Plymouth theme pulls PNG from role files; payload copy for audit)
mkdir -p "$WORK/seed/kevantic-payload/iso/boot-splash"
cp -a "$ROOT/iso/boot-splash/." "$WORK/seed/kevantic-payload/iso/boot-splash/"
...
if compgen -G "$OFFLINE_SRC/*.deb" >/dev/null; then
  ...
  mkdir -p "$WORK/seed/kevantic-payload/offline-packages/bin" "$WORK/seed/kevantic-payload/offline-packages/wheels"
  cp -a "$OFFLINE_SRC"/*.deb "$WORK/seed/kevantic-payload/offline-packages/"
  [[ -d "$OFFLINE_SRC/bin" ]] && cp -a "$OFFLINE_SRC/bin/." "$WORK/seed/kevantic-payload/offline-packages/bin/" || true
  [[ -d "$OFFLINE_SRC/wheels" ]] && cp -a "$OFFLINE_SRC/wheels/." "$WORK/seed/kevantic-payload/offline-packages/wheels/" || true
  [[ -f "$OFFLINE_SRC/SHA256SUMS" ]] && cp -f "$OFFLINE_SRC/SHA256SUMS" "$WORK/seed/kevantic-payload/offline-packages/"
  [[ -f "$OFFLINE_SRC/MANIFEST.txt" ]] && cp -f "$OFFLINE_SRC/MANIFEST.txt" "$WORK/seed/kevantic-payload/offline-packages/"
else
...
if [[ ! -f "$WORK/Kevantic-Appliance-Install.iso" ]]; then
  echo "ISO build failed — output missing" >&2
  exit 3
fi

cp -f "$WORK/Kevantic-Appliance-Install.iso" "$OUT_ISO"
(
  cd "$OUT_DIR"
