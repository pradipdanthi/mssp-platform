"""OTA staging + apply for Kevantic appliance (signed manifests)."""
...
LOG = logging.getLogger("kevantic-ota")
...
def ota_root() -> Path:
    return Path(os.environ.get("KEVANTIC_OTA_DIR", str(state_root() / "ota")))


def trust_pubkey() -> Path:
    env = os.environ.get("KEVANTIC_OTA_PUBKEY", "").strip()
    if env:
        return Path(env)
    return Path("/etc/kevantic/trust/keys/licensing-ed25519-v1.pub")
...
def verify_manifest_signature(manifest: Dict[str, Any]) -> bool:
    ...
    When signature blank (dev), accept only if KEVANTIC_OTA_ALLOW_UNSIGNED=1.
    """
    sig = (manifest.get("signature") or "").strip()
    if not sig or sig.startswith("REPLACE"):
        allow = (os.environ.get("KEVANTIC_OTA_ALLOW_UNSIGNED") or "").lower() in (
            "1",
            "true",
            "yes",
        )
    ...
    try:
        from kevantic_cli.license_ops import verify_and_parse, find_pubkey
...
def stage_offer(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    manifest = {
        ...
        "component": payload.get("component") or "kevantic-appliance-meta",
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p = argparse.ArgumentParser(prog="kevantic-ota")
