#!/usr/bin/env bash
# Export Proxmox build VM disk to versioned Kevantic field artifacts.
# Default: pull compressed qcow2 only (VM 100 disk is limited). Optional raw via env.
set -euo pipefail
...
NAME="Kevantic-Appliance-v${VER}"
STAGE_REMOTE="/var/tmp/kevantic-export-${VMID}"
# Pulling a full 40G raw fills VM 100 — off by default. Set KEVANTIC_EXPORT_RAW=1 to pull.
EXPORT_RAW="${KEVANTIC_EXPORT_RAW:-0}"
...
rm -rf "$STAGE_ISO"
mkdir -p "$STAGE_ISO"
cp -f "$DIST/${NAME}.qcow2" "$STAGE_ISO/"
cat > "$STAGE_ISO/README.txt" <<EOF
Kevantic Appliance v${VER}
========================
Built on Proxmox factory VM ${VMID} (not nested Packer on VM 100).
Single appliance image (minimize, kevantic-cli, DuckDB/Parquet engine,
...
Bare metal:    on a machine with space, convert then dd:
  qemu-img convert -f qcow2 -O raw ${NAME}.qcow2 ${NAME}.raw
  sudo dd if=${NAME}.raw of=/dev/DISK bs=4M status=progress conv=fsync

First boot: kevantic-cli setup → bootstrap update → network lock
EOF

if command -v genisoimage >/dev/null; then
  genisoimage -V "KEVANTIC_APPLIANCE" -J -r -o "$DIST/${NAME}.iso" "$STAGE_ISO"
elif command -v mkisofs >/dev/null; then
  mkisofs -V "KEVANTIC_APPLIANCE" -J -r -o "$DIST/${NAME}.iso" "$STAGE_ISO"
else
  docker run --rm -v "$DIST:/d" -w /d ubuntu:24.04 bash -lc \
    "apt-get update -qq && apt-get install -y -qq genisoimage >/dev/null && genisoimage -V KEVANTIC_APPLIANCE -J -r -o /d/${NAME}.iso /d/_iso_stage"
fi
rm -rf "$STAGE_ISO"

(
  cd "$DIST"
  sha256sum "${NAME}.qcow2" "${NAME}.iso" > SHA256SUMS
  [[ -f "${NAME}.raw" ]] && sha256sum "${NAME}.raw" >> SHA256SUMS
)
