...
OUT="${1:-$ROOT/.cache/output/kevantic-appliance-qcow2}"
VER="$(tr -d '[:space:]' < "$ROOT/VERSION")"
DIST="$ROOT/.cache/dist"
mkdir -p "$DIST"

QCOW=$(find "$OUT" -type f \( -name '*.qcow2' -o -name 'kevantic-appliance*' \) 2>/dev/null | head -1 || true)
if [[ -z "${QCOW}" ]]; then
...
elif docker image inspect kevantic-appliance-b2-builder:local >/dev/null 2>&1; then
  docker run --rm -v "$DIST:/d" kevantic-appliance-b2-builder:local \
...
if command -v genisoimage >/dev/null || command -v mkisofs >/dev/null || docker image inspect kevantic-appliance-b2-builder:local >/dev/null 2>&1; then
...
========================
Single appliance image (all features: minimize, kevantic-cli, DuckDB/Parquet
...
EOF
  if command -v genisoimage >/dev/null; then
    genisoimage -V "KEVANTIC_APPLIANCE" -J -r -o "$DIST/${NAME}.iso" "$STAGE"
  elif command -v mkisofs >/dev/null; then
    mkisofs -V "KEVANTIC_APPLIANCE" -J -r -o "$DIST/${NAME}.iso" "$STAGE"
  else
