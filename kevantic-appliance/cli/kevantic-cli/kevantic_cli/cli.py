"""kevantic-cli entrypoint (KB-093 B1 + Track-1 register/heartbeat)."""
...
from kevantic_cli import __version__, bootstrap, license_ops, network, register_ops, state
...
def cmd_version(_: argparse.Namespace) -> int:
    print(f"kevantic-cli {__version__}")
    print(
        f"appliance_train "
...
def cmd_channel(args: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "unit_installed": Path("/etc/systemd/system/kevantic-channeld.service").is_file(),
        "status_file": str(status_path),
        "status": None,
        "hint": "systemctl status kevantic-channeld; heartbeat timer remains Phase A fallback",
    }
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if not args.token:
        ...
        return 1
    app["appliance_name"] = args.appliance_name or app.get("appliance_name") or "kevantic-appliance"
    ...
    msg = {
        ...
        "next": [
            "kevantic-cli bootstrap update",
            "kevantic-cli network lock --yes",
            "kevantic-cli register --token <ACTIVATION_TOKEN>",
            "systemctl enable --now kevantic-heartbeat.timer",
        ],
...
def cmd_bootstrap(args: argparse.Namespace) -> int:
    ...
    print("usage: kevantic-cli bootstrap {status|update}", file=sys.stderr)
    return 1
...
def cmd_network(args: argparse.Namespace) -> int:
    if args.network_cmd == "status":
        _out(
            {
                "network_mode": state.get_network_mode(),
                "agent_source_cidrs": network.load_agent_cidrs(),
        ...
        return 0
    if args.network_cmd == "allow-agents":
        raw = (args.cidrs or "").strip()
        if not raw:
            print("error: --cidrs required (comma-separated IPv4 CIDRs)", file=sys.stderr)
            return 1
        parts = [p.strip() for p in raw.replace(";", ",").split(",") if p.strip()]
        try:
            result = network.apply_agent_cidrs(parts, dry_run=args.dry_run)
        except Exception as exc:  # noqa: BLE001
            print(f"error: {exc}", file=sys.stderr)
            return 4
        _out(result, args.json)
    ...
    if args.network_cmd == "unlock":
        ...
        return 0
    print(
        "usage: kevantic-cli network {status|allow-agents|lock|unlock}",
        file=sys.stderr,
    )
    return 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="kevantic-cli",
        description="Kevantic Appliance management CLI",
        parents=[common],
    )
    ...
    net_allow = net_sub.add_parser(
        "allow-agents",
        help="Open Wazuh agent ports (1514/1515) for one or more LAN CIDRs",
        parents=[common],
    )
    net_allow.add_argument(
        "--cidrs",
        required=True,
        help="Comma-separated IPv4 CIDRs (e.g. 10.10.0.0/16,192.168.0.0/24)",
    )
    net_allow.add_argument("--dry-run", action="store_true")
